...
### 51. 动态加载资源的方式？他们之间的区别 [?]
1.Resources.Load();

2.AssetBundle

### 52. 请描述游戏动画有哪几种，以及其原理？
...
### 59. Unity3D是否支持写成多线程程序？如果支持的话需要注意什么？
...
### 68. 已知strcpy函数的原型是：char * strcpy(char * strDest,const char * strSrc); 1.不调用库函数，实现strcpy函数。2.解释为什么要返回char * [?]

### 69. C#中四种访问修饰符是哪些？各有什么区别？
...
### 72. 请写出求斐波那契数列任意一位的值得算法

### 73. 协同程序的执行代码是什么？有何用处，有何缺点？
...
### 83. UNITY3d在移动设备上的一些优化资源的方法 [?]

### 84. 将Camera组件的ClearFlags选项选成Depth only是什么意思？有何用处？
如果把摄像机的ClearFlags勾选为Deapth Only,那么摄像机就会只渲染看得见的对象，把背景会完全透明，这种情况一般用在两个摄像机以上的场景中

### 85. 在编辑场景时将GameObject设置为Static有何作用？
设置游戏对象为Static时，这些部分被静态物体挡住而不可见时，将会剔除（或禁用）网格对象。因此，在你的场景中的所有不会动的物体都应该标记为Static。

标记成static的物体可以被光照渲染以及navMesh烘焙。

### 86. 有A和B两组物体，有什么办法能够保证A组物体永远比B组物体先渲染？
把A组物体的渲染对列大于B物体的渲染队列，通过shader里面的渲染队列来渲染。

### 87. 将图片的TextureType选项分别选为““Texture”和“Sprite”有什么区别
Sprite作为UI精灵使用，Texture作用模型贴图使用。Sprite需要2的整次幂，打包图片省资源

### 88. 问一个Terrain，分别贴3张，4张，5张地表贴图，渲染速度有什么区别？为什么？
没有区别，因为不管几张贴图只渲染一次。

### 89. 什么是DrawCall？DrawCall高了又什么影响？如何降低DrawCall？
...
### 95. 能用foreach遍历访问的对象需要实现接口或声明___方法的类型
...
隐藏对象的属性和实现细节，对外提供公共访问的接口。

封装原则：将不需要对外提供的内容，属性都隐藏起来，提供公共方法对其进行访问。

封装的例子：使用private修饰符将类的属性进行私有化，并提供公共方法使外部对其进行访问。

封装通过访问修饰符来实现，访问修饰符定义了一个类成员范围和可见性。

+ public：所有对象都可以访问
+ private：对象本身在对象内部可以访问，只有同一个类中的函数可以访问，类的实例也不能访问其私有成员
+ protected：只有该类的对象以及子类可以进行访问，有助于实现继承
+ internal：同一个程序集的对象可以访问


**继承：**

重用现有代码。允许通过一个现有类(基类)来定义一个新的类(派生类)。
...
多重继承是指一个类可以同时从多于一个父类继承行为与特征。C#不支持多重继承，但是可以通过接口来实现。

**多态：**

多态是指同一个行为拥有不同表现形式的能力。

多态性往往表示为 “一个接口，多个功能”。

多态性可以是静态的和动态的。静态多态性中，函数的响应是在编译时发生的，动态多态中，函数的响应是在运行时发生。

**C#中，每个类型都是多态的，因为所有类都继承自object类。**

多态就是同一个接口，使用不同的实例而执行不同操作。

**静态多态性**：包括函数重载以及操作符重载

函数重载(overload)：相同函数名，不同函数定义，可以是参数的数量不同，也可以是参数的类型不同。

不能重载只有返回类型不同的函数声明。

**动态多态性**：使用关键字abstract创建抽象类，用于提供接口的部分类的实现。当一个类继承自该抽象类时，实现即完成。

动态多态使用过抽象类和虚方法实现的。

抽象类包含抽象方法，抽象方法不能在抽象类中实现。抽象方法可以被派生类实现。

+ 无法创建抽象类的实例
+ 不能在抽象类外部声明一个抽象方法
+ 抽象类不能被sealed修饰，因为sealed修饰的类无法被继承。
+ 派生类重写(override)抽象类方法

**虚方法**：当一个定义在类中的函数需要在继承类中实现时，可以使用virtual关键字定义该函数。

虚方法在不同的继承类中有不同的实现。虚方法的调用是在运行时发生的，所以是动态多态。

## 2. C#中抽象类(abstract)，虚方法(virtual)，接口(interface)的不同

**virtual, abstract**:

1. virtual方法和abstract方法都可以在抽象类中定义，但是abstract方法不可以在抽象类中进行实现，必须要在他的子类中用override对函数的实体进行重写。abstract只定义来一个函数的名称，而这个函数功能的实现是要在继承该抽象类中的子类中实现的。

virtual方法可以在抽象类中进行定义，同时可以在抽象类中实现该方法，并且如果在抽象类中实现了该方法，在继承该抽象类的子类中依然可以用override重新构造该函数的函数实体。

2. abstract只能用在抽象类中，不可以用在非抽象类中。如果某一个类中有抽象函数，那么这个类必须被定义为抽象类，否则编译不会通过。virtual函数可以用在非抽象类中，并且该非抽象类的子类可以通过override进行重写，以实现子类特有的功能。

**virtual, abstract的具体用法**:

virtual方法是可以在抽象类中实现的，而abstract方法是不可以在抽象类中进行实现。

abstract方法主要是对具有相同方法但是不同实现方式的函数而言。比如动物的发声方式，有的动物用喉咙发声，有的动物使用翅膀摩擦发声，两种发声方式有较大的区别。

virtual方法主要是对相同方法并且相同表现形式的函数而言的。比如男人和女人具有相同喝水的方法，但具体实现有略微不同。

**abstract和interface的区别**：

abstract和interface都是通过定义一个类然后通过子类来进行实现。因为c#中不允许多重继承，也就是说一个子类最多只能只有一个父类。

如果需要子类继承多个父类中的不同方法，就可以在interface中声明这些方法，然后让子类继承这些interface，并在子类中具体实现这些方法。

## 3. 重载与重写的不同
**定义：**

重载：overload, 是相同函数名具有不同的函数参数类型，参数长度，参数顺序。调用时根据不同的参数调用不同的函数。返回值不同不能作为重载的依据。

重写：override, 是基类和派生类中对于虚函数的重写。函数拥有相同的函数名，参数。但是在基类和派生类中有不同的具体实现。

**定义范围：**

重载是在类中实现的，而重写是基类和派生类之间的关系。
...
+ 参数列表必须相同
+ 返回类型必须相同
+ 重写方法不能抛出新的检查异常或者比原函数更宽泛的检查型异常

**类关系上的区别：** 重载是在类中发生，所以是平行关系；而重写发生在父与子类的关系中，所以是垂直关系。
